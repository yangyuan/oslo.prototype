...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run Prototype's test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./prototype/tests/unit"
    fi
  ...
  if [ setup.cfg -nt prototype.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='prototype/*' --omit='prototype/openstack/common/*' -d covhtml -i
  fi
...
if [ $just_pep8_changed -eq 1 ]; then
    ...
    # file *names* and there are no files named (eg) "prototype/compute/manager.py".  The
